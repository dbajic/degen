...
def degenerate(n,k):
	...
	stars = int(sum([misc.comb(x,2) for x in G.degree().values()])) # How many total 2-stars are in the graph
...
if __name__ == "__main__":
	if (len(sys.argv)) >= 5:
			...
			if ('2-star' in sys.argv):
				models.append("2-star")		
			...
			for g in graphs:
				...
				if ('degseq' in sys.argv): #Degree Sequence model
					model += g[2] + "\t"
				if ('2-star' in sys.argv): #2-star model
					model += g[3] + "\t"
				print(model)
